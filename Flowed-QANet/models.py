...
class CQAttention(nn.Module):
    ...
    def forward(self, C, Q, Cmask, Qmask):
        ...
        # print('helli',Q.shape,C.shape)
        C = C.transpose(1, 2)
        Q = Q.transpose(1, 2)
       
        batch_size_c = C.size()[0]
        S = self.trilinear_for_attention(C, Q)
        Cmask = Cmask.view(batch_size_c, C_size, 1)
        Qmask = Qmask.view(batch_size_c, 1, Q_size)
    ...
    def trilinear_for_attention(self, C, Q):
        ...
        Q_size=Q.shape[1]
        C_size=C.shape[1]
        # print(C.shape,Q.shape[1])
        # print('------')
        # print(Lq,Lc)
       
        subres0 = torch.matmul(C, self.w4C).expand([-1, -1, Q_size])
        # print("========",torch.matmul(C, self.w4C).shape[1],torch.matmul(Q, self.w4Q).transpose(1,2).shape)
        subres1 = torch.matmul(Q, self.w4Q).transpose(1, 2).expand([-1,C_size, -1])
        subres2 = torch.matmul(C * self.w4mlu, Q.transpose(1,2))
        # print(self.w4C.shape,self.w4Q.shape,self.w4mlu.shape)
        # print(subres0.shape,subres1.shape[2],subres2.shape)
...
class QANet(nn.Module):
    ...
    def forward(self, Cwid, Ccid, Qwid, Qcid):
        ...
        # print('Cwbegin')
        # print(C.shape,Q.shape,maskC.shape,maskQ.shape)
        # print('cwover')
